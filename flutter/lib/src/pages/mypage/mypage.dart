...
import 'func/users_delete_func.dart';
import 'func/users_update_lang_func.dart';
...
class _MypageState extends State<Mypage> {
  ...
  void initState() {
    super.initState();
    final api = MyPageApi();
    api.fetchMyPageInfo(14).then((info) {
      setState(() {
        myPageInfo = info;
      });
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: myPageInfo == null
          ? Center(child: CircularProgressIndicator())
          : _myPageWidget(context, myPageInfo!),
    );
  }
        ...
        children: [
          ...
          Container(
            ...
            child: ListTile(
              ...
              subtitle: Column(
                children: myPageInfo!.dislikedMaterials
                    .map((material) => Text('${material.id}: ${material.name}'))
                    .toList(),
              ),
            ),
          ),
          ...
          Container(
            ...
            child:  ListTile(
              title: Text('History'),
              subtitle: Column(
                children: myPageInfo!.histories
                    .map((history) => Text('Action: ${history.action}, Date: ${history.date}'))
                    .toList(),
              ),
            ),
          ),
