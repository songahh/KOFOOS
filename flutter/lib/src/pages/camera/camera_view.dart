...
class _CameraViewState extends State<CameraView> with WidgetsBindingObserver {
  ...
  Future loadModel() async {
    String pathImageModel = "assets/ai/model_classification.pt";
    String pathObjectDetectionModel = "assets/ai/best.torchscript";
    try {
      _imageModel = await PytorchLite.loadClassificationModel(
          pathImageModel, 224, 224, 1000,
          labelPath: "assets/ai/label_classification_imageNet.txt");
  ...
  //     // 앱이 백그라운드로 전환될 때 카메라 스트림 중지
  //     await cameraController?.stopImageStream();
  //   } else if (state == AppLifecycleState.resumed) {
  //     // 앱이 다시 활성화될 때, 카메라가 스트리밍 중이지 않다면 스트림 재시작
